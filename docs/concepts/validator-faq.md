...
This key is created from `fury` and used to sign transactions. Application keys are associated with a public key prefixed by `blackap` and an address prefixed by `blackaa`. Both are derived from account keys generated by `fury keys add`.

Note: A validator's operator key is directly tied to an application key, but uses reserved prefixes solely for this purpose: `iva` and `blackvp`.
...
- Validator operator address, which is an [Application Key](#application-key) that you used to create the validator
  ...
  To query a validator's operator address(iva...) and pubkey(blackvp...):
