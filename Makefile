...
testnet-init:
	@if ! [ -f build/nodecluster/node0/fury/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/home fanfury/fanfury:furyhub fury testnet --v 4 --output-dir /home/nodecluster --chain-id highbury_710-1 --keyring-backend test --starting-ip-address 192.168.10.2 ; fi
