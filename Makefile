...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./lite/statik/statik.go" -not -path "*.pb.go" | xargs goimports -w -local github.com/fanfury/fanfury:furyhub
...
testnet-init:
	@if ! [ -f build/nodecluster/node0/fury/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/home fanfury/fanfury:furyhub fury testnet --v 4 --output-dir /home/nodecluster --chain-id fanfury_4200-1 --keyring-backend test --starting-ip-address 192.168.10.2 ; fi
